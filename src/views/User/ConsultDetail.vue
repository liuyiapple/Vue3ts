  ...
  <div class="consult-detail-page" v-if="item">
    ...
    <div class="detail-patient">
      <van-cell-group :border="false">
        ...
        />
        <van-cell title="病情描述" :value="`${item.illnessDesc}`" />
      </van-cell-group>
    </div>
    ...
    <div class="detail-time" v-if="item.status === OrderType.ConsultPay">
      请在 <van-count-down :time="item.countdown" /> 内完成支付，超时订单将取消
    </div>
    <div
    ...
    >
      ...
      <van-button
        ...
        >取消问诊</van-button
      >
      <van-button type="primary" size="small" round @click="show = true"
        >继续支付</van-button
      >
    </div>
    <!-- 带接诊 -->
    ...
    >
      ...
      <van-button
        ...
        >问诊记录</van-button
      >
      <van-button type="default" size="small" round v-if="item.evaluateId">
        查看评价
      </van-button>
      <van-button type="primary" size="small" round v-else> 写评价 </van-button>
    </div>
    <!-- 取消 -->
    <div
      class="detail-action van-hairline--top"
    ...
    >
      <van-button
        ...
        >删除订单</van-button
      >
      <van-button type="primary" size="small" round :to="`/home`">
        咨询其他医生
      </van-button>
    </div>
    <PaySheet
...
const { deleteConsultOrder, deleteLoading } = useDeleteOrder(() => {
